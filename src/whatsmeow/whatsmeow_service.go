...
func (source *WhatsmeowServiceModel) GetLogger() log.Logger {
...
func Start(options WhatsmeowOptions, dbParameters library.DatabaseParameters, logentry log.Logger) {
	...
	if len(options.LogLevel) > 0 {
		...
		logentry = logentry.WithLevel(WhatsmeowLogLevel)
	}
