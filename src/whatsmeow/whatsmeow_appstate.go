...
// sendAppState sends an app state patch to WhatsApp (no retry, returns error as-is)
func sendAppState(conn *WhatsmeowConnection, patch appstate.PatchInfo) error {
	ctx := context.Background()
	return conn.Client.SendAppState(ctx, patch)
}

// MarkChatAsRead marks a chat as read using the app state protocol
...
// MarkChatAsUnread marks a chat as unread using the app state protocol
...
// ArchiveChat archives or unarchives a chat using the app state protocol
...
// SaveContact saves a contact to the WhatsApp address book.
// When syncToPhone is true, it also syncs to the device's phone contacts
// (equivalent to "Sync contact with phone" in WhatsApp Web).
