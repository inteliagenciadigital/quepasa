...
func DepackSplitRed(p []byte, log ...qplog.Logger) ([]MlowFrame, error) {
	...
	n := len(p)
	if n == 0 {
		lg.DebugE().Msg("red depack: empty packet")
		return nil, ErrPktSizeZero
	}
	lg.TraceE().Int("packet_bytes", n).Msg("red depack")
	...
	lg.TraceE().Int("redundant_blocks", len(red)).Int("main_bytes", mainSize).Int("total_frames", len(frames)).Msg("red depack: done")
	return frames, nil
}
