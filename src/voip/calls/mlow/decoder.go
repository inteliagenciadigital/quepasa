...
func (d *MlowDecoder) Decode(payload []byte) []float32 {
	...
	if len(payload) == 0 {
		d.log.TraceE().Msg("decode: empty payload, emitting silence")
		return make([]float32, opusFrameSamps)
	}
	d.log.TraceE().Int("payload_bytes", len(payload)).Int32("redundancy", d.redundancy).Msg("decode packet")
	if d.redundancy > 0 {
		frames, err := DepackSplitRed(payload, d.log)
		if err != nil {
			d.log.DebugE().Err(err).Int("payload_bytes", len(payload)).Msg("decode: RED depack failed, emitting silence")
			return make([]float32, opusFrameSamps)
		}
		var main []byte
		if len(frames) > 0 {
		...
		d.log.TraceE().Int("red_frames", len(frames)).Int("main_bytes", len(main)).Msg("decode: RED depacked")
...
func (d *MlowDecoder) decodeFrame(frame []byte) []float32 {
	...
	if len(frame) == 0 {
		d.log.TraceE().Msg("decode frame: empty, emitting silence")
		return make([]float32, opusFrameSamps)
	}
	...
	d.log.TraceE().Int("frame_bytes", len(frame)).Uint8("toc_byte", frame[0]).
	...
	if toc.StdOpus {
		d.log.DebugE().Msg("decode frame: standard-Opus packet, not handled, emitting silence")
		return make([]float32, outLen)
	}
	if toc.SID || !toc.Active {
		d.log.TraceE().Bool("sid", toc.SID).Bool("active", toc.Active).Msg("decode frame: inactive/SID, emitting silence")
		return make([]float32, outLen)
	}
...
func (d *MlowDecoder) decodeActiveFrame(frame []byte, outLen int) []float32 {
	...
	d.log.TraceE().Int("config", config).Bool("low_rate", lowRate).Int("body_bytes", len(frame)-1).Int("internal_frames", 3).Msg("decode active frame")
	...
	for f := 0; f < 3; f++ {
		...
		d.log.TraceE().Int("intf", f).Bool("voiced", voiced).Int32("stage1", lsf.Stage1).
	...
	plen := len(out)
	d.log.TraceE().Int("samples", plen).Int("packet_lags", len(packetLags)).Msg("decode active frame: applying harmonic postfilter")
