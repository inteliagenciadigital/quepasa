...
func ParseSmplTOC(b byte, log ...qplog.Logger) SmplTOC {
	...
	lg := pickLog(log)
	if b&0xC0 == 0xC0 {
		lg.TraceE().Uint8("toc_byte", b).Bool("std_opus", true).Msg("parse toc: standard-Opus packet")
		return SmplTOC{
			StdOpus:    true,
			SampleRate: 16000,
	...
	lg.TraceE().Uint8("toc_byte", b).Bool("sid", toc.SID).Bool("vad", toc.VAD).
