...
func EncodeSmplFrame(fp *SmplFrameParams, log ...qplog.Logger) ([]byte, error) {
	...
	p6 := int32(fp.Config)
	lg.TraceE().Uint8("toc_byte", fp.TOC).Int("config", fp.Config).Int("internal_frames", 3).Msg("encode frame")
	tbl := LoadSmplTables()
	mem := LoadSmplMem()
	...
	for f := 0; f < 3; f++ {
		ip := &fp.Internal[f]
		lg.TraceE().Int("intf", f).Bool("voiced", ip.Lsf.Stage1 == 1).Int32("stage1", ip.Lsf.Stage1).
	...
	enc.Done()
	if enc.Err() != 0 {
		lg.DebugE().Int32("err", enc.Err()).Msg("encode frame: range-encoder buffer overflow")
	...
	lg.TraceE().Int("frame_bytes", len(out)).Int("body_bytes", n).Msg("encode frame: done")
	return out, nil
}
...
func (e *MlowEncoder) Encode(pcm []float32) ([]byte, error) {
	...
	if len(pcm) != opusFrameSamps {
		e.log.DebugE().Int("samples", len(pcm)).Int("want", opusFrameSamps).Msg("encode: wrong frame size")
		return nil, errors.New("mlow encode: expected 960 samples (60 ms @16 kHz)")
	}
	e.log.TraceE().Int("samples", len(pcm)).Msg("encode frame: sanitizing and analyzing")
