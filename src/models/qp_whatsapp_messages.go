...
func (source *QpWhatsappMessages) writeRecord(id string, record cache.MessageRecord) bool {
	if source.backend == nil {
		qplog.Errorf("message cache backend not initialized")
		return false
	}

	err := source.backend.Set(source.buildKey(id), record)
	if err != nil {
		qplog.Errorf("failed to persist message cache record: %v", err)
		return false
	}
...
func (source *QpWhatsappMessages) listRecords() []cache.MessageRecordEntry {
	...
	if err != nil {
		qplog.Errorf("failed to list message cache records: %v", err)
		return nil
	}
...
func (source *QpWhatsappMessages) SetStatusById(id string, status whatsapp.WhatsappMessageStatus) {
	...
	record, found, err := source.readRecord(normalizedId)
	if err != nil {
		qplog.Errorf("failed to read message status record: %v", err)
		return
	}
