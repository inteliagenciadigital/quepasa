...
// LogLevelFromConfig parses the environment's log level string into a qplog.Level.
...
func (*Environment) LogLevelFromConfig(defaultLevel qplog.Level) qplog.Level {
	envLevelStr := ENV.LogLevel()
	if len(envLevelStr) == 0 {
		return defaultLevel
	}

	parsedLevel, err := qplog.ParseLevel(envLevelStr)
	if err != nil {
