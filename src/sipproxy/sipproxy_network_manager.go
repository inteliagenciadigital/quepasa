...
func NewSIPProxyNetworkManager(settings SIPProxyNetworkManagerSettings, logentry qplog.Logger) *SIPProxyNetworkManager {
...
func (snm *SIPProxyNetworkManager) discoverLocalIPWithSipgo() (string, error) {
	...
	snm.logger.WithError(err).Infof("UDP4 routing method failed, trying network interface detection for IPv4")
