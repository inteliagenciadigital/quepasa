...
func NewSIPProxySTUNManager(logger qplog.Logger, configuredServer string) *SIPProxySTUNManager {
...
func (sm *SIPProxySTUNManager) DiscoverPublicIPv4WithFallback() (string, error) {
	...
	if sm.configuredServer != "" && sm.configuredServer != ":0" {
		sm.logger.WithField("stun_server", sm.configuredServer).Debugf("Trying configured STUN server")
		if ip, err := sm.discoverIPFromServer(sm.configuredServer); err == nil {
			sm.logger.WithFields(qplog.Fields{
				"public_ip":   ip,
				"stun_server": sm.configuredServer,
			}).Infof("Public IP discovered using configured STUN server, IP: %s", ip)
			return ip, nil
		} else {
			sm.logger.WithError(err).WithField("stun_server", sm.configuredServer).Warnf("Failed to discover IP using configured server, trying fallback")
		}
	} else {
		sm.logger.Debugf("No configured STUN server or empty value, using fallback servers")
	}

	// Try fallback servers
	for _, server := range sm.fallbackServers {
		sm.logger.WithField("stun_server", server).Debugf("Trying fallback STUN server")
		if ip, err := sm.discoverIPFromServer(server); err == nil {
			...
			sm.logger.WithError(err).WithField("stun_server", server).Debugf("Failed with this fallback server")
		}
	}
...
func (sm *SIPProxySTUNManager) discoverIPFromServer(stunServer string) (string, error) {
	sm.logger.WithField("stun_server", stunServer).Debugf("Attempting IPv4 STUN discovery")
	...
	if err := mappedAddr.GetFrom(response); err != nil {
		...
		if ipv4 := regularAddr.IP.To4(); ipv4 != nil {
			...
			}).Debugf("Successfully discovered public IPv4 via STUN (regular address)")
			return publicIP, nil
		} else {
	...
	if ipv4 := mappedAddr.IP.To4(); ipv4 != nil {
		...
		sm.logger.WithFields(qplog.Fields{
			"public_ipv4": publicIP,
			"stun_server": stunServer,
		}).Debugf("Successfully discovered public IPv4 via STUN (XOR mapped address)")
		return publicIP, nil
	} else {
		return "", fmt.Errorf("STUN server %s returned IPv6 address, but IPv4 is required", stunServer)
	}
