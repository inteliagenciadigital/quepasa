...
// expandEnvVariables expands ${VAR} and $VAR references in all loaded environment variables
...
func init() {
	logentry := qplog.New().WithField("package", "environment")
...
func getEnvOrDefaultInt(key string, defaultValue int) int {
	if valueStr, ok := os.LookupEnv(key); ok {
		...
		qplog.Warnf("Invalid integer value for environment variable %s: '%s'. Using default: %d", key, valueStr, defaultValue)
	}
	return defaultValue
}
...
func getOptionalEnvInt(key string) int {
	if valueStr, ok := os.LookupEnv(key); ok {
		...
		qplog.Warnf("Invalid integer value for environment variable %s: '%s'. Feature will be disabled", key, valueStr)
	}
