...
- ✅ Investigation tracking slice: created `docs/INVESTIGATION-whatsapp-send-suspensions-and-destination-validation.md` to capture evidence-first analysis of post-2026-05-13 suspension spike, with explicit classification criteria for local identifier resolution vs remote recipient-existence probing in send hot path
- ✅ Investigation evidence slice: confirmed `QpWhatsappServer.SendMessage` performs conditional pre-send `IsOnWhatsApp` checks when `NORMALIZE_BR_PHONE`/`REMOVEDIGIT9` is enabled (introduced in commit `9d723b2`, 2026-05-06); confirmed whatsmeow dependency bump to `c551a4055c0f` occurred in commit `d422433` on 2026-05-13; report updated in `docs/INVESTIGATION-whatsapp-send-suspensions-and-destination-validation.md`
- ✅ Investigation evidence slice: confirmed `REMOVEDIGIT9=false` default in `src/.env.example` (feature is runtime-env gated), and identified secondary lookup amplifier in current whatsmeow (`c551a4055c0f`) where send path may call `GetUserInfo` for PN->LID resolution when `LIDMigrationTimestamp > 0` and store mapping miss occurs; report updated in `docs/INVESTIGATION-whatsapp-send-suspensions-and-destination-validation.md`
- ✅ Investigation evidence slice: confirmed runtime service profile on current host loads `/opt/quepasa/.env` and currently has `REMOVEDIGIT9=true`, meaning conditional pre-send `IsOnWhatsApp` path is effectively enabled in live service context; report updated in `docs/INVESTIGATION-whatsapp-send-suspensions-and-destination-validation.md`
- ✅ Investigation evidence slice: operational log probe over `/var/log/quepasa` and `journalctl -u quepasa` did not expose lookup-specific traces with current patterns/log level; observability gap noted in investigation report
